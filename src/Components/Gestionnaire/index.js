...
class RecharcheStagiairesGestionnaire extends Component {
  constructor(props) {
    super(props);
    this.state = {
    ...
    this.handleChangeDropDownFormateur =
      this.handleChangeDropDownFormateur.bind(this);
    this.handleChangeDropDownGroupe =
      this.handleChangeDropDownGroupe.bind(this);
  ...
  checkApiStagiaires = (sg) => {
    ...
    fetch(
      "http://localhost/backend/stagiaires/?token=" +
        this.state.SELECTEDFORMATEURHash +
        "&groupe=" +
        sg,
      requestOptions
    )
  ...
  handleClickBtnRechercher() {
    ...
    let EstMarque = "No";
    if (
      this.state.SELECTEDFORMATEUREmail === "" ||
      ...
      if (
        ...
        if (
          ...
          this.state.ABSENCES.map((item) => {
            if (
              item.Formateur === this.state.SELECTEDFORMATEUREmail &&
          ...
          if (EstMarque === "No") {
            ...
            if (
              !moment(sDate).isBefore(
                moment(new Date()).format("YYYY-MM-DD"),
                "day"
              )
            ) {
              Toast.success(
                "L'ABSENCE N'EST PAS ENCORE MARUQUE OU AUCUN N'EST PAS ABSENTE",
                3000,
                () => {
              ...
              Toast.fail(
                "AUCUN ABSENCE EST MARQUEE ET VOUS POUVEZ PAS MARQUE L'ABSENCE DANS CETTE EMPLOI",
                3000,
                () => {}
              );
            }
          ...
          <Table compact celled definition>
            <Table.Header>
              <Table.Row className="mt">
                <Table.Cell className="text-center " colSpan="5">
                  <Label size="huge" color="blue" horizontal className="mt-3">
                    Gestion Absence
                  </Label>
                </Table.Cell>
              </Table.Row>
            ...
            <Table.Body>
              ...
              <Table.Row>
                <Table.Cell colSpan="4">
                  <Button
                    onClick={this.handleClickBtnRechercher}
...
export default connect(mapStateToProps, mapDispatchToProps)(RecharcheStagiairesGestionnaire);