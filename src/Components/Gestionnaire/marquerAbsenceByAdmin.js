...
class MarquerAbsenceByAdmin extends Component {
  constructor(props) {
    super(props);
    ...
    this.handleChangeSliderStagiaires =
      this.handleChangeSliderStagiaires.bind(this);
  ...
  handleChangeSliderStagiaires(event, item) {
    ...
    if (item.checked === true)
      this.setState({
        SELECTEDSTG: [
          ...this.state.SELECTEDSTG,
          document.getElementById("NumInscription" + item.id).textContent,
        ],
      });
    else if (item.checked === false)
      this.setState({
        SELECTEDSTG: this.state.SELECTEDSTG.filter(
          (items) =>
            items !==
            document.getElementById("NumInscription" + item.id).textContent
        ),
  ...
  bodyFunc() {
    ...
    if (this.checkinfos() === true) {
      if (
        this.state.Stagiaires === "undefined" ||
        this.state.Stagiaires === undefined ||
        this.state.Stagiaires === 0
      ) {
        return (
          <Table.Row>
          ...
          return (
            <Table.Row key={i}>
              <Table.Cell textAlign="center">
                <Checkbox
                  onChange={this.handleChangeSliderStagiaires}
                  slider
                  id={i + ""}
                />
              </Table.Cell>
              ...
              <Table.Cell textAlign="center" id={"NumInscription" + i}>
                {item.NumInscription}
              </Table.Cell>
            </Table.Row>
          );
              ...
              <Table.Row>
                ...
                >
                  <Button
                    ...
                    onClick={this.handleClickBtnMarquer}
                  >
                    Marquer L'absence
                  </Button>
                </Table.HeaderCell>
...
export default MarquerAbsenceByAdmin;
